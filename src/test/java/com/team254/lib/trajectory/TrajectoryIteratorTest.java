...
public class TrajectoryIteratorTest {
  ...
  public void test() {
    ...
    TrajectoryIterator<Translation2d, Rotation2d254> iterator =
        new TrajectoryIterator<>(traj.getIndexView());
    ...
    // Advance forward.
    Assert.assertEquals(
        kWaypoints.get(0).interpolate(kWaypoints.get(1), 0.5), iterator.preview(0.5).state()
    );
    Assert.assertEquals(
        kHeadings.get(0).interpolate(kHeadings.get(1), 0.5), iterator.preview(0.5).heading()
    );
    TrajectorySamplePoint<Translation2d, Rotation2d254> newPoint = iterator.advance(0.5);
    ...
    // Advance backwards.
    Assert.assertEquals(
        kWaypoints.get(0).interpolate(kWaypoints.get(1), 0.25), iterator.preview(-0.25).state()
    );
    Assert.assertEquals(
        kHeadings.get(0).interpolate(kHeadings.get(1), 0.25), iterator.preview(-0.25).heading()
    );

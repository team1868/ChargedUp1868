...
public class TimingUtilTest {
  ...
  public static final List<Rotation2d254> kHeadings = List.of(
  ...
  public <S extends State<S>, T extends State<T>> Trajectory<TimedState<S>, TimedState<T>>
  buildAndCheckTrajectory(
    ...
    Trajectory<TimedState<S>, TimedState<T>> timed_traj = TimingUtil.timeParameterizeTrajectory(
        false, dist_view, step_size, constraints, start_vel, end_vel, max_vel, max_acc
    );
  ...
  public <S extends State<S>, T extends State<T>> void checkTrajectory(
    ...
    for (int i = 0; i < traj.length(); ++i) {
      ...
      for (final TimingConstraint<S> constraint : constraints) {
        Assert.assertTrue(
            state.velocity() - kTestEpsilon <= constraint.getMaxVelocity(state.state())
        );
        final MinMaxAcceleration accel_limits =
            constraint.getMinMaxAcceleration(state.state(), state.velocity());
  ...
  public void testNoConstraints() {
    ...
    DistanceView<Translation2d, Rotation2d254> dist_view = new DistanceView<>(traj);

    // Triangle profile.
    Trajectory<TimedState<Translation2d>, TimedState<Rotation2d254>> timed_traj =
        buildAndCheckTrajectory(
            dist_view, 1.0, new ArrayList<TimingConstraint<Translation2d>>(), 0.0, 0.0, 20.0, 5.0
        );
    System.out.println(timed_traj.toCSV());

    // Trapezoidal profile.
    timed_traj = buildAndCheckTrajectory(
        dist_view, 1.0, new ArrayList<TimingConstraint<Translation2d>>(), 0.0, 0.0, 10.0, 5.0
    );
    System.out.println(timed_traj.toCSV());

    // Trapezoidal profile with start and end velocities.
    timed_traj = buildAndCheckTrajectory(
        dist_view, 1.0, new ArrayList<TimingConstraint<Translation2d>>(), 5.0, 2.0, 10.0, 5.0
    );
    System.out.println(timed_traj.toCSV());
  }
  ...
  public void testConditionalVelocityConstraint() {
    ...
    class ConditionalTimingConstraint<S extends ITranslation2d<S>> implements TimingConstraint<S> {
      ...
      public TimingConstraint.MinMaxAcceleration getMinMaxAcceleration(S state, double velocity) {
        return new TimingConstraint.MinMaxAcceleration(
    ...
    Trajectory<TimedState<Translation2d>, TimedState<Rotation2d254>> timed_traj =
        buildAndCheckTrajectory(
            dist_view, 1.0, Arrays.asList(new ConditionalTimingConstraint<>()), 0.0, 0.0, 10.0, 5.0
        );
    System.out.println(timed_traj.toCSV());
  }
  ...
  public void testVelocityLimitRegionConstraint() {
    ...
    // Trapezoidal profile.
    Trajectory<TimedState<Translation2d>, TimedState<Rotation2d254>> timed_traj =
        buildAndCheckTrajectory(dist_view, 1.0, Arrays.asList(constraint), 0.0, 0.0, 10.0, 5.0);
    System.out.println(timed_traj.toCSV());
  }
}

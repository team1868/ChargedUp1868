...
public class TrajectoryTest {
  ...
  List<Rotation2d254> kHeadings = Arrays.asList(
      Rotation2d254.fromDegrees(0),
      Rotation2d254.fromDegrees(30),
      Rotation2d254.fromDegrees(60),
      Rotation2d254.fromDegrees(90),
      Rotation2d254.fromDegrees(180)
  );

  @Test
  public void testConstruction() {
    // Empty constructor.
    Trajectory<Translation2d, Rotation2d254> traj = new Trajectory<>();
  ...
  public void testStateAccessors() {
    Trajectory<Translation2d, Rotation2d254> traj = new Trajectory<>(kWaypoints, kHeadings);
    ...
    Assert.assertEquals(
        kWaypoints.get(0).interpolate(kWaypoints.get(1), .25), traj.getInterpolated(0.25).state()
    );
    Assert.assertEquals(traj.getInterpolated(0.25).index_floor(), 0);
    Assert.assertEquals(traj.getInterpolated(0.25).index_ceil(), 1);
    Assert.assertEquals(
        kWaypoints.get(1).interpolate(kWaypoints.get(2), .5), traj.getInterpolated(1.5).state()
    );
    Assert.assertEquals(traj.getInterpolated(1.5).index_floor(), 1);
    Assert.assertEquals(traj.getInterpolated(1.5).index_ceil(), 2);
    Assert.assertEquals(
        kWaypoints.get(2).interpolate(kWaypoints.get(3), .75), traj.getInterpolated(2.75).state()
    );
    Assert.assertEquals(traj.getInterpolated(2.75).index_floor(), 2);
    Assert.assertEquals(traj.getInterpolated(2.75).index_ceil(), 3);

    Assert.assertEquals(
        kHeadings.get(0).interpolate(kHeadings.get(1), .25), traj.getInterpolated(0.25).heading()
    );
    Assert.assertEquals(traj.getInterpolated(0.25).index_floor(), 0);
    Assert.assertEquals(traj.getInterpolated(0.25).index_ceil(), 1);
    Assert.assertEquals(
        kHeadings.get(1).interpolate(kHeadings.get(2), .5), traj.getInterpolated(1.5).heading()
    );
    Assert.assertEquals(traj.getInterpolated(1.5).index_floor(), 1);
    Assert.assertEquals(traj.getInterpolated(1.5).index_ceil(), 2);
    Assert.assertEquals(
        kHeadings.get(2).interpolate(kHeadings.get(3), .75), traj.getInterpolated(2.75).heading()
    );
    Assert.assertEquals(traj.getInterpolated(2.75).index_floor(), 2);
    Assert.assertEquals(traj.getInterpolated(2.75).index_ceil(), 3);

    Trajectory<Translation2d, Rotation2d254>.IndexView index_view = traj.getIndexView();
    Assert.assertEquals(
        kWaypoints.get(0).interpolate(kWaypoints.get(1), .25), index_view.sample(0.25).state()
    );
    Assert.assertEquals(
        kWaypoints.get(1).interpolate(kWaypoints.get(2), .5), index_view.sample(1.5).state()
    );
    Assert.assertEquals(
        kWaypoints.get(2).interpolate(kWaypoints.get(3), .75), index_view.sample(2.75).state()
    );
    Assert.assertEquals(
        kHeadings.get(0).interpolate(kHeadings.get(1), .25), index_view.sample(0.25).heading()
    );
    Assert.assertEquals(
        kHeadings.get(1).interpolate(kHeadings.get(2), .5), index_view.sample(1.5).heading()
    );
    Assert.assertEquals(
        kHeadings.get(2).interpolate(kHeadings.get(3), .75), index_view.sample(2.75).heading()
    );
  }
}

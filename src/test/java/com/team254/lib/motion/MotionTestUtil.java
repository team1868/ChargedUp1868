...
public class MotionTestUtil {
  ...
  public static MotionState followProfile(
      ProfileFollower follower, Dynamics dynamics, double dt, int max_iterations
  ) {
    int i = 0;
    for (; i < max_iterations && !follower.onTarget(); ++i) {
      ...
      System.out.println(
          ...
          + ", Vel error: " + follower.getVelError() + ", Command: " + command_vel
      );

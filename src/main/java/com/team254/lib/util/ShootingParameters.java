...
public class ShootingParameters {
  ...
  public enum BallQuality { NEW_BALL, MEDIUM_BALL, OLD_BALL }

  public ShootingParameters(
      InterpolatingTreeMap<InterpolatingDouble, InterpolatingDouble> oldBallHoodMap,
  ...
  isShooterAtSetpoint(double current_shooter_rpm, double shooter_setpoint) {
    return Util.epsilonEquals(current_shooter_rpm, shooter_setpoint, mShooterAllowableErrorRPM);
  }

  public synchronized boolean
  isTurretAtSetpoint(double current_turret_angle, double turret_setpoint, double range_meters) {
    ...
    final double meters_error =
        Math.tan(Units.degrees_to_radians(angle_diff_degrees)) * range_meters;

...
public class MotionProfile {
  ...
  public boolean isValid() {
    ...
    for (MotionSegment s : mSegments) {
      ...
      if (prev_segment != null && !s.start().coincident(prev_segment.end())) {
        ...
        System.err.println(
            "Segments not continuous! End: " + prev_segment.end() + ", Start: " + s.start()
        );
        return false;
      }
      prev_segment = s;
  ...
  public void appendControl(double acc, double dt) {
    ...
    MotionState new_start_state =
        new MotionState(last_end_state.t(), last_end_state.pos(), last_end_state.vel(), acc);
    appendSegment(
        new MotionSegment(new_start_state, new_start_state.extrapolate(new_start_state.t() + dt))
    );
  }

...
/**
 * A LinearTimeVaryingMotionProfileGoal defines a goal with a constant velocity whose position is a
 * function of an initial position, the velocity, and time. In other words, the goal state is
 * constantly moving according to a time varying linear function.
 */
public class LinearTimeVaryingMotionProfileGoal implements IMotionProfileGoal {
  ...
  public LinearTimeVaryingMotionProfileGoal(
      double t, double initial_goal_position, double goal_vel
  ) {
    this.mT = t;
  ...
  public LinearTimeVaryingMotionProfileGoal flipped() {
    return new LinearTimeVaryingMotionProfileGoal(
        mT, -mInitialGoalPosition, -mGoalVel, mCompletionBehavior, mPosTolerance, mVelTolerance
    );
  }
  ...
  /**
   * Create a MotionProfileGoal that has a zero velocity goal state, for use with superposition to
   * work with a moving reference frame.
   *

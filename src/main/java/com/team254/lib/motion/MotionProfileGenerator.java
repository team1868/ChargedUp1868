...
public class MotionProfileGenerator {
  ...
  protected static MotionProfile generateFlippedProfile(
    ...
    MotionProfile profile =
        generateProfileImpl(constraints, goal_state.flipped(), prev_state.flipped(), !origFlipped);
  ...
  public synchronized static MotionProfile generateProfile(
      MotionProfileConstraints constraints, IMotionProfileGoal goal_state, MotionState prev_state
  ) {
    if (goal_state instanceof MotionProfileGoal) {
      return generateProfile(constraints, (MotionProfileGoal) goal_state, prev_state);
    } else {
      return generateProfile(
          constraints, (LinearTimeVaryingMotionProfileGoal) goal_state, prev_state
      );
    }
  }
   ...
   */
  public synchronized static MotionProfile generateProfile(
      MotionProfileConstraints constraints, MotionProfileGoal goal_state, MotionState prev_state
  ) {
  ...
  public synchronized static MotionProfile generateProfile(
    ...
    if (goal_state.max_vel() >= constraints.max_vel()
        || goal_state.max_vel() <= constraints.min_vel()) {
      // Reaching the goal exactly is impossible, so just move towards the goal as quickly as
      // possible.
      return generateProfile(
          constraints,
          new MotionProfileGoal(
              Math.signum(goal_state.max_vel()) * Double.MAX_VALUE, goal_state.max_vel()
          ),
          prev_state
      );
    }

    // Turn the linear time-varying goal into a stationary goal by superimposing the (opposite of)
    // goal state motion onto the initial state and all constraints. Then add the goal state motion
    // back to the result.
    MotionState superposedPrevState = new MotionState(
        prev_state.t(), prev_state.pos(), prev_state.vel() - goal_state.max_vel(), prev_state.acc()
    );
    ...
    var profile =
        generateProfileImpl(superposedConstraints, superposedGoal, superposedPrevState, false);
    if (profile.size() == 0)
      return profile;
    ...
    // As a result of the shift, individual segments may now (a) be able to be combined, (b) have to
    // be split in order to ensure that no segment has both positive and negative velocities.
    ...
    for (var segment : profile.segments()) {
      if (segment.start().vel() * segment.end().vel() >= 0.0) {
        ...
        // Velocity sign changes midway through segment. Figure out where and make two new segments
        // around it.
  ...
  protected static MotionState shiftByVelocity(
      MotionState state, double velocity_shift, double start_time
  ) {
    return new MotionState(
        state.t(),
        state.pos() + velocity_shift * (state.t() - start_time),
        state.vel() + velocity_shift,
        state.acc()
    );
  }

  protected static MotionProfile generateProfileImpl(
      MotionProfileConstraints constraints,
      MotionProfileGoal goal_state,
      MotionState prev_state,
      boolean flipped
    ...
    if (delta_pos < 0.0 || (delta_pos == 0.0 && prev_state.vel() < 0.0)) {
      ...
      // to solve, then flip the solution.
    ...
    MotionState start_state = new MotionState(
        prev_state.t(),
        prev_state.pos(),
    ...
    if (min_abs_vel_at_goal_sqr > 0.0
      ...
      if (goal_state.completion_behavior() == CompletionBehavior.VIOLATE_MAX_ABS_VEL) {
        ...
        if (Math.abs(delta_pos) < goal_state.pos_tolerance()) {
          ...
          // acceleration, which will result in NaNs below, so we can return the profile
          // immediately.
          profile.appendSegment(new MotionSegment(
              ...
              new MotionState(
                  profile.endTime(), profile.endPos(), goal_vel, Double.NEGATIVE_INFINITY
              )
          ));
          profile.consolidate();
          return profile;
        ...
        profile.appendProfile(
            generateFlippedProfile(constraints, goal_state, profile.endState(), flipped)
        );
        profile.consolidate();
        return profile;
      }
    ...
    // accelerating towards the goal from profile.finalState(), the other coming from the goal at
    // max vel (in reverse). If Vmax is greater than constraints.max_abs_vel, we will clamp and
    // cruise. Solve the following three equations to find Vmax (by substitution): Vmax^2 = Vstart^2
    // + 2*a*d_accel Vgoal^2 = Vmax^2 - 2*a*d_decel delta_pos = d_accel + d_decel
    final double v_max = Math.min(
        max_vel, Math.sqrt((start_state.vel2() + goal_vel * goal_vel) / 2.0 + delta_pos * max_acc)
    );
    ...
    final double distance_decel = Math.max(
        0.0, (start_state.vel2() - goal_vel * goal_vel) / (2.0 * constraints.max_abs_acc())
    );

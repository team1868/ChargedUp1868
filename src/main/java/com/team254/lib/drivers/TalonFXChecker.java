...
public class TalonFXChecker extends MotorChecker<BaseTalon> {
  ...
  protected ArrayList<TalonFXChecker.StoredTalonFXConfiguration> mStoredConfigurations =
      new ArrayList<>();
  ...
  protected void storeConfiguration() {
    ...
    for (MotorChecker.MotorConfig<BaseTalon> config : mMotorsToCheck) {
      ...
      TalonFXChecker.StoredTalonFXConfiguration configuration =
          new TalonFXChecker.StoredTalonFXConfiguration();
  ...
  protected void restoreConfiguration() {
    for (int i = 0; i < mMotorsToCheck.size(); ++i) {
      mMotorsToCheck.get(i).mMotor.set(
          mStoredConfigurations.get(i).mMode, mStoredConfigurations.get(i).mSetValue
      );
    }
  }

...
public abstract class ServoMotorSubsystem extends Subsystem {
  ...
  protected ServoMotorSubsystem(final ServoMotorSubsystemConstants constants) {
    ...
    TalonUtil.checkError(
        mMaster.configSelectedFeedbackSensor(
            FeedbackDevice.IntegratedSensor, 0, mConstants.kLongCANTimeoutMs
        ),
        mConstants.kName + ": Could not detect encoder: "
    );

    mForwardSoftLimitTicks = (int
    ) ((mConstants.kMaxUnitsLimit - mConstants.kHomePosition) * mConstants.kTicksPerUnitDistance);
    TalonUtil.checkError(
        mMaster.configForwardSoftLimitThreshold(
            mForwardSoftLimitTicks, mConstants.kLongCANTimeoutMs
        ),
        mConstants.kName + ": Could not set forward soft limit: "
    );

    TalonUtil.checkError(
        mMaster.configForwardSoftLimitEnable(true, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": Could not enable forward soft limit: "
    );

    mReverseSoftLimitTicks = (int
    ) ((mConstants.kMinUnitsLimit - mConstants.kHomePosition) * mConstants.kTicksPerUnitDistance);
    TalonUtil.checkError(
        mMaster.configReverseSoftLimitThreshold(
            mReverseSoftLimitTicks, mConstants.kLongCANTimeoutMs
        ),
        mConstants.kName + ": Could not set reverse soft limit: "
    );

    TalonUtil.checkError(
        mMaster.configReverseSoftLimitEnable(true, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": Could not enable reverse soft limit: "
    );

    TalonUtil.checkError(
        mMaster.configVoltageCompSaturation(12.0, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": Could not set voltage compensation saturation: "
    );

    TalonUtil.checkError(
        mMaster.config_kP(kMotionProfileSlot, mConstants.kKp, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": could not set kP: "
    );

    TalonUtil.checkError(
        mMaster.config_kI(kMotionProfileSlot, mConstants.kKi, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": could not set kI: "
    );

    TalonUtil.checkError(
        mMaster.config_kD(kMotionProfileSlot, mConstants.kKd, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": could not set kD: "
    );

    TalonUtil.checkError(
        mMaster.config_kF(kMotionProfileSlot, mConstants.kKf, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": Could not set kF: "
    );

    TalonUtil.checkError(
        mMaster.configMaxIntegralAccumulator(
            kMotionProfileSlot, mConstants.kMaxIntegralAccumulator, mConstants.kLongCANTimeoutMs
        ),
    ...
    TalonUtil.checkError(
        mMaster.config_IntegralZone(
            kMotionProfileSlot, mConstants.kIZone, mConstants.kLongCANTimeoutMs
        ),
    ...
    TalonUtil.checkError(
        mMaster.configAllowableClosedloopError(
            kMotionProfileSlot, mConstants.kDeadband, mConstants.kLongCANTimeoutMs
        ),
        mConstants.kName + ": Could not set deadband: "
    );

    TalonUtil.checkError(
        mMaster.config_kP(kPositionPIDSlot, mConstants.kPositionKp, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": could not set kP: "
    );

    TalonUtil.checkError(
        mMaster.config_kI(kPositionPIDSlot, mConstants.kPositionKi, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": could not set kI: "
    );

    TalonUtil.checkError(
        mMaster.config_kD(kPositionPIDSlot, mConstants.kPositionKd, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": could not set kD: "
    );

    TalonUtil.checkError(
        mMaster.config_kF(kPositionPIDSlot, mConstants.kPositionKf, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": Could not set kF: "
    );
    ...
    TalonUtil.checkError(
        mMaster.config_IntegralZone(
            kPositionPIDSlot, mConstants.kPositionIZone, mConstants.kLongCANTimeoutMs
        ),
        mConstants.kName + ": Could not set i zone: "
    );
    ...
    TalonUtil.checkError(
        mMaster.configAllowableClosedloopError(
            kPositionPIDSlot, mConstants.kPositionDeadband, mConstants.kLongCANTimeoutMs
        ),
        mConstants.kName + ": Could not set deadband: "
    );
    ...
    TalonUtil.checkError(
        mMaster.configMotionCruiseVelocity(
            mConstants.kCruiseVelocity, mConstants.kLongCANTimeoutMs
        ),
        mConstants.kName + ": Could not set cruise velocity: "
    );

    TalonUtil.checkError(
        mMaster.configMotionAcceleration(mConstants.kAcceleration, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": Could not set acceleration: "
    );

    TalonUtil.checkError(
        mMaster.configOpenloopRamp(mConstants.kRampRate, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": Could not set voltage ramp rate: "
    );

    TalonUtil.checkError(
        mMaster.configClosedloopRamp(mConstants.kRampRate, mConstants.kLongCANTimeoutMs),
        mConstants.kName + ": Could not set closed loop ramp rate: "
    );

    TalonUtil.checkError(
        mMaster.configSupplyCurrentLimit(new SupplyCurrentLimitConfiguration(
    ...
    mMaster.setStatusFramePeriod(
        StatusFrameEnhanced.Status_8_PulseWidth, mConstants.kStatusFrame8UpdateRate, 20
    );
    ...
    for (int i = 0; i < mSlaves.length; ++i) {
      mSlaves[i] = TalonFXFactory.createPermanentSlaveTalon(
          mConstants.kSlaveConstants[i].id, mConstants.kMasterConstants.id
      );
    ...
    // The accel term can re-use the velocity unit conversion because both input and output units
    // are per second.
  ...
  protected enum ControlState { OPEN_LOOP, MOTION_MAGIC, POSITION_PID, MOTION_PROFILING }
  ...
  public synchronized void readPeriodicInputs() {
    ...
    if (mMaster.getControlMode() == ControlMode.MotionMagic) {
      ...
      if (mPeriodicIO.active_trajectory_position < mReverseSoftLimitTicks) {
        DriverStation.reportError(
            mConstants.kName + ": Active trajectory past reverse soft limit!", false
        );
      } else if (mPeriodicIO.active_trajectory_position > mForwardSoftLimitTicks) {
        DriverStation.reportError(
            mConstants.kName + ": Active trajectory past forward soft limit!", false
        );
      }
      final double newVel = mMaster.getActiveTrajectoryVelocity();
      if (Util.epsilonEquals(newVel, mConstants.kCruiseVelocity, Math.max(1, mConstants.kDeadband))
          || Util.epsilonEquals(
              newVel, mPeriodicIO.active_trajectory_velocity, Math.max(1, mConstants.kDeadband)
          )) {
        ...
        mPeriodicIO.active_trajectory_acceleration =
            Math.signum(newVel - mPeriodicIO.active_trajectory_velocity) * mConstants.kAcceleration;
      }
    ...
    if (mConstants.kRecoverPositionOnReset) {
      mPeriodicIO.absolute_pulse_position_modded =
          (int) mMaster.getSensorCollection().getIntegratedSensorAbsolutePosition();
      ...
      double estimated_pulse_pos =
          ((mConstants.kMasterConstants.invert_sensor_phase ? -1 : 1) * mPeriodicIO.position_ticks)
          + mPeriodicIO.absolute_pulse_offset;
      int new_wraps = (int
      ) Math.floor(estimated_pulse_pos / ((double) mConstants.kMasterConstants.encoder_ppr));
  ...
  protected int getAbsoluteEncoderRawPosition() {
    int abs_raw_with_rollover =
        (int) mMaster.getSensorCollection().getIntegratedSensorAbsolutePosition();
    return abs_raw_with_rollover
        + (abs_raw_with_rollover < 0
               ? abs_raw_with_rollover + mConstants.kMasterConstants.encoder_ppr
               : 0);
  }
    ...
    mEnabledLooper.register(new Loop() {
      ...
      public void onLoop(double timestamp) {
        if (mPeriodicIO.reset_occured) {
          System.out.println(
              mConstants.kName + ": Master Talon reset occurred; resetting frame rates."
          );
          mMaster.setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, 5, 20);
          ...
          if (mConstants.kRecoverPositionOnReset) {
            mMaster.setSelectedSensorPosition(
                estimateSensorPositionFromAbsolute(), 0, mConstants.kCANTimeoutMs
            );
          }
        }
  ...
  public synchronized void setSetpointPositionPID(double units, double feedforward_v) {
    ...
    mPeriodicIO.feedforward =
        feedforward_ticks_per_100ms * (mConstants.kKf + mConstants.kKd / 100.0) / 1023.0;
  ...
  public synchronized void setSetpointMotionProfiling(
      IMotionProfileGoal goal, double feedforward_v
  ) {
    if (mControlState != ControlState.MOTION_PROFILING) {
      mMaster.selectProfileSlot(kPositionPIDSlot, 0);
      mControlState = ControlState.MOTION_PROFILING;
      mMotionStateSetpoint = new MotionState(
    ...
    mPeriodicIO.feedforward =
        (unitsPerSecondToTicksPer100ms(feedforward_v + setpoint.motion_state.vel()) * mConstants.kKf
         + unitsPerSecondToTicksPer100ms(setpoint.motion_state.acc()) * mConstants.kKa)
        / 1023.0;
  ...
  public int estimateSensorPositionFromAbsolute() {
    int estimated_pulse_pos = (mPeriodicIO.encoder_wraps * mConstants.kMasterConstants.encoder_ppr)
        + mPeriodicIO.absolute_pulse_position_modded;
    int estimate_position_ticks = (mConstants.kMasterConstants.invert_sensor_phase ? -1 : 1)

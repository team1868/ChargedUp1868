...
public class TrajectoryUtil {
  public static <S extends IPose2d<S>, T extends IRotation2d<T>> Trajectory<S, T> mirror(
      final Trajectory<S, T> trajectory
  ) {
  ...
  public static <S extends IPose2d<S>, T extends IRotation2d<T>>
      Trajectory<TimedState<S>, TimedState<T>> mirrorTimed(
          final Trajectory<TimedState<S>, TimedState<T>> trajectory
      ) { // todo fix
  ...
  public static <S extends IPose2d<S>, T extends IRotation2d<T>> Trajectory<S, T> transform(
      final Trajectory<S, T> trajectory, Pose2d transform
  ) {
    List<S> waypoints = new ArrayList<>(trajectory.length());
    List<T> headings = new ArrayList<>(trajectory.length());
    for (int i = 0; i < trajectory.length(); ++i) {
  ...
  public static <S extends State<S>, T extends State<T>> Trajectory<S, T> resample(
    ...
    for (int i = 0; i < num_states; ++i) {
      ...
      headings.add(
          trajectory_view.sample(i * interval + trajectory_view.first_interpolant()).heading()
      );
    }
    return new Trajectory<>(states, headings);
  }

  public static Trajectory<Pose2dWithCurvature, Rotation2d254> trajectoryFromWaypoints(
      final List<Pose2d> waypoints,
      final List<Rotation2d254> headings,
      double maxDx,
      double maxDy,
      double maxDTheta
  ) {
  ...
  public static Trajectory<Pose2dWithCurvature, Rotation2d254> trajectoryFromSplinesAndHeadings(
    ...
    return new Trajectory<>(
        SplineGenerator.parameterizeSplines(splines, headings, maxDx, maxDy, maxDTheta)
    );
  }
}

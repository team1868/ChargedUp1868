...
public class TimingUtil {
  public static <S extends State<S>, T extends State<T>> Trajectory<TimedState<S>, TimedState<T>>
  timeParameterizeTrajectory(
    ...
    for (int i = 0; i < num_states; ++i) {
      ...
      headings.add(
          distance_view.sample(Math.min(i * step_size, distance_view.last_interpolant())).heading()
      );
    }
  ...
  timeParameterizeTrajectory(
    ...
    // velocity parameterized (though we may adjust the velocity downwards during the backwards
    // pass). We wish to find an acceleration that is admissible at both the start and end state, as
    // well as an admissible end velocity. If there is no admissible end velocity or acceleration,
    // we set the end velocity to the state's maximum allowed velocity and will repair the
    ...
    for (int i = 0; i < states.size(); ++i) {
      ...
      // acceleration limits may be a function of velocity.
      while (true) {
        // Enforce global max velocity and max reachable velocity by global acceleration limit.
        // vf = sqrt(vi^2 + 2*a*d)
        constraint_state.max_translational_velocity = Math.min(
            max_translational_velocity,
            Math.sqrt(
                predecessor.max_translational_velocity * predecessor.max_translational_velocity
        ...
        // from predecessor state max accel.
        ...
        for (final TimingConstraint<S> constraint : constraints) {
          ...
          constraint_state.min_translational_acceleration = Math.max(
              constraint_state.min_translational_acceleration,
        ...
        // applied, we need to reduce the max accel at the predecessor state and try again.
        // TODO: Simply using the new max acceleration is guaranteed to be valid, but may be too
        // conservative. Doing a search would be better.
    ...
    for (int i = states.size() - 1; i >= 0; --i) {
      ...
      while (true) {
        ...
        final double new_max_translational_velocity = Math.sqrt(
            successor.max_translational_velocity * successor.max_translational_velocity
        ...
        if (constraint_state.min_translational_acceleration > actual_acceleration + kEpsilon) {
          successor.min_translational_acceleration =
              constraint_state.min_translational_acceleration;
        } else {
    ...
    for (int i = 0; i < states.size(); ++i) {
      ...
      final double accel = (constrained_state.max_translational_velocity
                                * constrained_state.max_translational_velocity
                            - v * v)
          / (2.0 * ds);
      double dt = 0.0;
      if (i > 0) {
      ...
      timed_headings.add(
          new TimedState<>(constrained_state.heading, t, reverse ? -v : v, reverse ? -accel : accel)
      ); // todo verify
    }
  ...
  protected static class ConstrainedState<S extends State<S>, T extends State<T>> {
    ...
    public String toString() {
      return state.toString() + ", distance: " + distance
          + ", max_translational_velocity: " + max_translational_velocity + ", "
          + "min_translational_acceleration: " + min_translational_acceleration

...
public class PurePursuitController<S extends ITranslation2d<S>, T extends IRotation2d<T>>
    implements IPathFollower {
  ...
  public PurePursuitController(
      final DistanceView<S, T> path, double sampling_dist, double lookahead, double goal_tolerance
  ) {
  ...
  public Twist2d steer(final Pose2d current_pose) {
    ...
    final Translation2d path_setpoint =
        current_pose.getTranslation().inverse().translateBy(iterator_.getState().getTranslation());
    final Rotation2d254 heading_setpoint =
        current_pose.getRotation().inverse().rotateBy(iterator_.getHeading().getRotation());
  ...
  public static class Arc<S extends ITranslation2d<S>> {
    ...
    protected double findLength(Pose2d pose, S point, Translation2d center, double radius) {
      if (radius < Double.MAX_VALUE) {
        ...
        // point is behind, check the sign of the cross-product between the normal vector and the
        // vector from pose to point.
        final boolean behind = Math.signum(Translation2d.cross(
                                   pose.getRotation().normal().toTranslation(),
                                   new Translation2d(pose.getTranslation(), point.getTranslation())
                               ))
            > 0.0;
        final Rotation2d254 angle = Translation2d.getAngle(centerToPose, centerToPoint);
        return radius

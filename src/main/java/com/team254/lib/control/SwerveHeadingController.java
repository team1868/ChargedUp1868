...
public class SwerveHeadingController {
  ...
  public enum HeadingControllerState {
    OFF,
    SNAP, // for snapping to specific headings
  ...
  public double calculateAngleToOrigin(Pose2d current_pose) {
    ...
    double theta_central =
        Math.atan2(centerOfGoal.getTranslation().y(), centerOfGoal.getTranslation().x());

    double angle = Math.toDegrees(
        Math.PI
        + Math.atan2(
            r * Math.sin(theta) - r_central * Math.sin(theta_central),
  ...
  public double update(double current_angle) {
    ...
    switch (mHeadingControllerState) {
      case OFF:
        return 0.0;
        //   case SNAP:
        //     mPIDFController.setPID(
        //         Constants.kSnapSwerveHeadingKp, Constants.kSnapSwerveHeadingKi,
        //         Constants.kSnapSwerveHeadingKd
        //     );
        //     break;
        //   case MAINTAIN:
        //     mPIDFController.setPID(
        //         Constants.kMaintainSwerveHeadingKp, Constants.kMaintainSwerveHeadingKi,

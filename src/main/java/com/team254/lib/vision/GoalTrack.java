 ...
 * destroy, or update a goal track. <p> This helps in the goal ranking process that determines which
 * goal to fire into, and helps to smooth measurements of the goal's location over time.
 *
 * @see GoalTracker
 */
public class GoalTrack {
  ...
  public synchronized boolean tryUpdate(double timestamp, Pose2d new_observation) {
    ...
    double distance =
        mSmoothedPosition.inverse()
            .transformBy(new_observation)
            .getTranslation()
  ...
  public static GoalTrack makeNewTrack(
      GoalTracker.Configuration config, double timestamp, Pose2d first_observation, int id
  ) {
  ...
  synchronized void smooth() {
    if (isAlive()) {
      ...
      if (num_samples == 0) {
        ...
        mSmoothedPosition = new Pose2d(x, y, new Rotation2d254(c, s, true));
      }
    }
  }
  ...
  public synchronized double getStability() {
    return Math.min(
        ...
        mObservedPositions.size()
            / (mConfiguration.kCameraFrameRate * mConfiguration.kMaxGoalTrackAge)
    );
  }

...
public class SplineGenerator {
   ...
   */
  public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d254>> parameterizeSpline(
      Spline s,
      List<? extends Rotation2d254> headings,
      double maxDx,
      double maxDy,
      double maxDTheta,
      double t0,
    ...
    List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d254>> rv = new ArrayList<>();
   ...
   */
  public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d254>> parameterizeSpline(
      Spline s, List<? extends Rotation2d254> headings
  ) {
    return parameterizeSpline(s, headings, kMaxDX, kMaxDY, kMaxDTheta, 0.0, 1.0);
  }

  public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d254>> parameterizeSpline(
      Spline s, List<? extends Rotation2d254> headings, double maxDx, double maxDy, double maxDTheta
  ) {
    return parameterizeSpline(s, headings, maxDx, maxDy, maxDTheta, 0.0, 1.0);
  }

  public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d254>> parameterizeSplines(
      List<Spline> splines, List<? extends Rotation2d254> headings
  ) {
  ...
  public static List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d254>> parameterizeSplines(
    ...
    if (splines.isEmpty())
      return rv;
    rv.add(new TrajectoryPoint<>(
        splines.get(0).getPose2dWithCurvature(0.0), headings.get(0).getRotation(), 0
    ));
    for (int i = 0; i < splines.size(); i++) {
      Spline s = splines.get(i);
      List<Rotation2d254> spline_rots = new ArrayList<>();
      ...
      List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d254>> samples =
          parameterizeSpline(s, spline_rots, maxDx, maxDy, maxDTheta);
      samples.remove(0);
      rv.addAll(samples);
    }
    return rv;
  ...
  private static void getSegmentArc(
      Spline s,
      List<? extends Rotation2d254> headings,
      List<TrajectoryPoint<Pose2dWithCurvature, Rotation2d254>> rv,
      double t0,
      double t1,
      double maxDx,
      double maxDy,
    ...
    Rotation2d254 r0 = s.getHeading(t0);
    Rotation2d254 r1 = s.getHeading(t1);
    ...
    if (twist.dy > maxDy || twist.dx > maxDx || twist.dtheta > maxDTheta) {
      ...
      // Interpolate heading
      Rotation2d254 diff = headings.get(1).rotateBy(headings.get(0).inverse());
      if (diff.getRadians() > Math.PI) {
        diff = diff.inverse().rotateBy(Rotation2d254.fromRadians(Math.PI));
      }
      Rotation2d254 interpolated_heading = headings.get(0).rotateBy(diff.times(t1 / totalTime));

      rv.add(
          new TrajectoryPoint<>(s.getPose2dWithCurvature(t1), interpolated_heading, rv.size() - 1)
      );
    }
  }
}

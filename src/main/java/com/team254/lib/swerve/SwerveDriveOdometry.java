...
public class SwerveDriveOdometry {
  ...
  private Rotation2d254 m_previousAngle;
  ...
  public Pose2d getPoseMetersPolar() {
    return new Pose2d(
        ...
        Math.toDegrees(
            Math.atan2(m_poseMeters.getTranslation().y(), m_poseMeters.getTranslation().x())
        ),
  ...
  public Pose2d updateWithTime(
      double currentTimeSeconds, Rotation2d254 gyroAngle, SwerveModuleState... moduleStates
  ) {
    ...
    var newPose = Pose2d.exp(new Twist2d(
  ...
  public Pose2d updateWithWheelConstraints(
    ...
    // m_velocity.omegaRadiansPerSecond = m_previousAngle.inverse().rotateBy(gyroAngle).getRadians()
    // / period;
    m_poseMeters = new Pose2d(m_poseMeters.transformBy(newPose).getTranslation(), angle);
    m_previousAngle = angle;
   ...
   * @return The new pose of the robot.
   */
  public Pose2d update(Rotation2d254 gyroAngle, SwerveModuleState[] moduleStates) {

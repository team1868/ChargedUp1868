...
public class SwerveModuleState implements Comparable<SwerveModuleState> {
  ...
  /** Angle of the module. */
  public Rotation2d254 angle = Rotation2d254.fromDegrees(0);

  public edu.wpi.first.math.kinematics.SwerveModuleState getWPI() {
    return new edu.wpi.first.math.kinematics.SwerveModuleState(
        speedMetersPerSecond, Rotation2d.fromDegrees(angle.getDegrees())
    );
  }
   ...
   */
  public SwerveModuleState(double speedMetersPerSecond, Rotation2d254 angle) {
    this.speedMetersPerSecond = speedMetersPerSecond;
    this.angle = angle;
  }

  public SwerveModuleState(
      double speedMetersPerSecond, double distanceMeters, Rotation2d254 angle
  ) {
    this.speedMetersPerSecond = speedMetersPerSecond;
  ...
  public static SwerveModuleState optimize(
      SwerveModuleState desiredState, Rotation2d254 currentAngle
  ) {
    var delta = desiredState.angle.rotateBy(currentAngle.inverse()); // todo check math
    if (Math.abs(delta.getDegrees()) > 90.0) {
      return new SwerveModuleState(
          -desiredState.speedMetersPerSecond,
          desiredState.angle.rotateBy(Rotation2d254.fromDegrees(180.0))
      );
    } else {

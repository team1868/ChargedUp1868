...
public class SwerveSetpointGenerator {
   ...
   *     direction.
   */
  private boolean flipHeading(Rotation2d254 prevToGoal) {
  ...
  protected double findDriveMaxS(
    ...
    // velocity at s=0: sqrt(x_0^2 + y_0^2) +/- max_vel_step = ...quadratic...
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    Rotation2d254[] prev_heading = new Rotation2d254[modules.length];
    double[] desired_vx = new double[modules.length];
    double[] desired_vy = new double[modules.length];
    Rotation2d254[] desired_heading = new Rotation2d254[modules.length];
    boolean all_modules_should_flip = true;
    for (int i = 0; i < modules.length; ++i) {
      prev_vx[i] = prevSetpoint.mModuleStates[i].angle.cos()
          * prevSetpoint.mModuleStates[i].speedMetersPerSecond;
      prev_vy[i] = prevSetpoint.mModuleStates[i].angle.sin()
          * prevSetpoint.mModuleStates[i].speedMetersPerSecond;
      ...
      desired_vx[i] =
          desiredModuleState[i].angle.cos() * desiredModuleState[i].speedMetersPerSecond;
      desired_vy[i] =
          desiredModuleState[i].angle.sin() * desiredModuleState[i].speedMetersPerSecond;
      ...
      if (all_modules_should_flip) {
        double required_rotation_rad =
            Math.abs(prev_heading[i].inverse().rotateBy(desired_heading[i]).getRadians());
    ...
    // lazy).
    List<Optional<Rotation2d254>> overrideSteering = new ArrayList<>(modules.length);
    ...
    for (int i = 0; i < modules.length; ++i) {
      ...
      if (Util.epsilonEquals(prevSetpoint.mModuleStates[i].speedMetersPerSecond, 0.0)) {
        ...
        var necessaryRotation =
            prevSetpoint.mModuleStates[i].angle.inverse().rotateBy(desiredModuleState[i].angle);
        if (flipHeading(necessaryRotation)) {
          necessaryRotation = necessaryRotation.rotateBy(Rotation2d254.kPi);
        }
        ...
        if (numStepsNeeded <= 1.0) {
          ...
          overrideSteering.set(
              i,
              Optional.of(prevSetpoint.mModuleStates[i].angle.rotateBy(Rotation2d254.fromRadians(
    ...
    for (int i = 0; i < modules.length; ++i) {
      ...
      final var deltaRotation =
          prevSetpoint.mModuleStates[i].angle.inverse().rotateBy(retStates[i].angle);
